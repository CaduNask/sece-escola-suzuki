...
useEffect(() => {
  async function loadPerfis() {
    ...
    const perfisAtivos = data.perfis
      .filter((perfil: any) => perfil.ativo === true)
        ...
        <main className={`${fontBody.className} relative z-10 flex min-h-dvh w-full flex-col bg-transparent text-[#123126] antialiased selection:bg-[#f0743e]/15`}>
          ...
          <div className="relative z-10 mx-auto flex w-full max-w-5xl flex-1 flex-col px-5 pb-16 pt-10 sm:px-8 sm:pb-20 sm:pt-12 md:px-12 lg:px-16 lg:pt-16">
            <div className="mx-auto w-full max-w-4xl">
              <p className={`suzuki-enter suzuki-enter-delay-2 ${fontDisplay.className} text-[0.65rem] font-medium uppercase tracking-[0.28em] text-[#123126]/72 sm:text-xs`}>
                Triagem inicial
              </p>
  
              <h1 className={`suzuki-enter suzuki-enter-delay-3 mt-5 max-w-2xl ${fontDisplay.className} text-[2.1rem] font-light leading-[1.08] tracking-[-0.02em] text-[#123126] sm:text-4xl md:text-[2.8rem]`}>
                Qual opção descreve melhor o seu momento?
              </h1>
  
              <p className="suzuki-enter suzuki-enter-delay-4 mt-7 max-w-xl text-[0.98rem] font-light leading-relaxed text-[#123126]/78 sm:text-base">
                A partir da sua escolha, a pesquisa segue por um caminho pensado
                especificamente para a fase em que você está.
              </p>
  
              <div className="suzuki-enter suzuki-enter-delay-5 mt-12 divide-y divide-[#123126]/10 border-y border-[#123126]/10">
  {isLoading ? (
    <div className="py-8">
      <p className={`${fontDisplay.className} text-[0.68rem] font-medium uppercase tracking-[0.24em] text-[#123126]/50`}>
        Carregando alternativas...
      </p>

      <p className="mt-3 max-w-md text-[0.92rem] font-light leading-relaxed text-[#123126]/55">
        Estamos preparando os caminhos disponíveis para a sua pesquisa.
      </p>
    </div>
  ) : (
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      const data = await getSuzukiData();
                        ...
                        const perfilEncontrado = data.perfis.find(
                          (item: any) => item.nome_perfil === perfil
                        );
                      
                        const pesquisasDoPerfil = data.regras
                          .filter(
                            (regra: any) =>
                              regra.id_perfil === perfilEncontrado?.id_perfil &&
                              regra.ativo === true
                          )
                          .map((regra: any) => regra.id_pesquisa);
                      
                        localStorage.setItem("perfilSelecionado", perfil);
                        localStorage.setItem("pesquisasDoPerfil", JSON.stringify(pesquisasDoPerfil));
                        localStorage.setItem("pesquisaAtualIndex", "0");
                      
                        await new Promise((resolve) => setTimeout(resolve, 400));

window.location.href = "/introducao";
                      }}
                      className={`group flex w-full items-center justify-between gap-6 py-6 text-left transition-all duration-500 ${
                        isNavigating
                          ? selectedPerfil === perfil
                            ? "pointer-events-none text-[#f0743e] opacity-100 blur-0"
                            : "pointer-events-none opacity-35 blur-[1px]"
                          : "hover:text-[#f0743e]"
                      }`}
                  >
                    <span className="flex items-start gap-8">
                      <span className={`${fontDisplay.className} mt-1 w-7 text-[0.68rem] font-medium tabular-nums tracking-[0.16em] text-[#123126]/42`}>
                        {String(index + 1).padStart(2, "0")}
                      </span>
  
                      <span className="text-[1.05rem] font-light leading-snug text-[#123126] transition-colors group-hover:text-[#f0743e]">
                        {perfil}
                      </span>
                    </span>
  
                    <span
  className={`text-[#123126]/35 transition-all duration-300 ${
    selectedPerfil === perfil && isNavigating
      ? "translate-x-1 text-[#f0743e]"
      : "group-hover:translate-x-1 group-hover:text-[#f0743e]"
  }`}
>
  {selectedPerfil === perfil && isNavigating ? "Preparando..." : "→"}
</span>
                  </button>
                  ))
                )}
